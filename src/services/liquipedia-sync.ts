...
export class LiquipediaSyncService {
  async fetchAndSyncMatches(): Promise<void> {
    try {
      ...
      try {
        ...
        logger.error(error, "Failed to fetch League of Legends matches:");
      }
      ...
      try {
        ...
        logger.error(error, "Failed to fetch Valorant matches:");
      }
      ...
      for (const match of allMatches) {
        try {
          ...
          logger.error(error, `Failed to process match ${match.match2id}:`);
        }
      }

      logger.info(`Sync completed: ${syncedCount} matches processed, ${updatedCount} updated`);
    } catch (error) {
      logger.error(error, "Failed to sync Liquipedia data:");
      throw error;
    }
  }
  ...
  private async upsertMatch(match: LiquipediaMatch): Promise<boolean> {
    ...
    try {
      ...
      // Ensure extradata contains the timestamp for proper time filtering
      const extraData = {
        ...match.extradata,
        timestamp: match.extradata?.timestamp || match.dateexact,
      };
      ...
      await db
        ...
        .values({
          ...
          team1Shortname:
            team1?.type === "team" ? team1?.teamtemplate?.shortname || team1?.name || null : null,
          ...
          team2Shortname:
            team2?.type === "team" ? team2?.teamtemplate?.shortname || team2?.name || null : null,
      ...
      logger.error(error, `Failed to upsert match ${match.match2id}:`);
      throw error;
    }
  }
  ...
  private async upsertTeam(opponent: any): Promise<number | null> {
    ...
    try {
      ...
      logger.error(error, `Failed to upsert team ${opponent.name}:`);
      return null;
    }
  }
