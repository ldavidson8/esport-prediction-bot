...
export async function sendEventMessage(
  ...
  const team1EmojiId = team1Emoji
    ? team1Emoji.match(/:(\d+)>/)?.[1]
    : undefined;
  const team2EmojiId = team2Emoji
    ? team2Emoji.match(/:(\d+)>/)?.[1]
    : undefined;
...
async function checkPastMatches(client: CustomClient) {
  ...
  for (const match of matches) {
    try {
      ...
      if (winner) {
        const score = `${matchData.teams[0].result.gameWins}-${matchData.teams[1].result.gameWins}`;
        ...
        try {
          ...
          logger.error(
            `Failed to update message for match ${match.id}:`,
            error
          );
        }
...
export function startScheduleProcessor(client: CustomClient) {
  // scheduleJob('*/10 * * * *', () => scheduleProcess(client)); // run every 10 minutes
